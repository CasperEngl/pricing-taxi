...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly create pricing-taxi
  fly create pricing-taxi-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app pricing-taxi
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app pricing-taxi-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'pricing-taxi-staging' does not match app name in config file 'pricing-taxi'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name pricing-taxi-db
  fly postgres attach --postgres-app pricing-taxi-db --app pricing-taxi

  fly postgres create --name pricing-taxi-staging-db
  fly postgres attach --postgres-app pricing-taxi-staging-db --app pricing-taxi-staging
  ```
